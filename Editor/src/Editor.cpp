...
Editor& Editor::GetInstance() {
  if (!_instance) _instance = std::unique_ptr<Editor>(new Editor());
  return *_instance;
}
...
void Editor::Init() {
  ...
  ImGui_ImplGlfw_InitForOpenGL((GLFWwindow*)mainWindow->GetNativeWindow(),
                               true);
  ImGui_ImplOpenGL3_Init(glsl_version);

  auto fex = std::make_shared<FileExplorer>(_game, 90, 720);
  auto contentWindow =
      std::make_shared<FileContentWindow>(_game, 720, 720, nullptr);
...
void Editor::Run() {
  ...
  while (isRunning) {
    // Logic updates
    isRunning &= mainWindow->OnUpdate(dt);
    for (auto w : windows) isRunning &= w->OnUpdate(dt);
...
std::shared_ptr<Bored::Scene> LoadMainScene(Manifest& manifest)
{
  // TODO: implement
  return nullptr;
}
...
void Editor::Shutdown() {
  ...
  for (auto w : windows) w->OnShutdown();
...
void Editor::OpenProject(const std::string& directory) {
  ...
  std::filesystem::path man_file =
      std::filesystem::path(directory) / "Bored.json";
  ...
  m_activeScene = LoadMainScene(_game);
}
