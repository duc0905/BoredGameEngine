...
void ClassicGamemode::SetupCamera()
{
    ...
    std::shared_ptr<Bored::Input::Context> inGame = std::make_shared<Bored::Input::Context>();
    inGame->AddRangeMapping(Bored::Input::KEY_A, 0, "MOVE_CAM_HOR", -1.0f);
...
void ClassicGamemode::SetupPieces(std::shared_ptr<Player> p, bool flip)
{
  p->pieces.clear();
  // 8 pawns
  for (int i = 0; i < 8; i++) {
