...
std::shared_ptr<Render::Texture> Renderer::LoadTexture(const std::string& name, unsigned int w, unsigned int h, unsigned int bpp, unsigned char* data) {
...
std::shared_ptr<Render::Texture> Renderer::GetTexture(const std::string& name) {
  if (_textureRegistry.find(name) == _textureRegistry.end()) return nullptr;
...
std::shared_ptr<Render::Mesh> Renderer::GetMesh(const std::string& name) {
  if (_meshRegistry.find(name) == _meshRegistry.end()) return nullptr;
...
std::shared_ptr<Render::Material> Renderer::GetMaterial(const std::string& name) {
  if (_materialRegistry.find(name) == _materialRegistry.end()) return nullptr;
...
void GetMeshes(const aiScene* scene, std::vector<std::shared_ptr<Mesh>>& meshes) {
  ...
  for (unsigned int i = 0; i < scene->mNumMeshes; i++) {
    ...
    for (unsigned int j = 0; j < me->mNumVertices; j++)
      myMesh->pos[j] = { me->mVertices[j].x, me->mVertices[j].y,
                             me->mVertices[j].z };

    // Only support 1 texture coordinate set for now
    if (me->HasTextureCoords(0)) {
      for (unsigned int j = 0; j < me->mNumVertices; j++)
        myMesh->uvs[j] = { me->mTextureCoords[0][j].x,
                              me->mTextureCoords[0][j].y };
    }

    if (me->HasNormals())
      for (unsigned int j = 0; j < me->mNumVertices; j++)
        myMesh->norms[j] = { me->mNormals[j].x, me->mNormals[j].y,
                                me->mNormals[j].z };
...
std::shared_ptr<Render::Model> Renderer::LoadModel(const std::string& file) {
  ...
  for (unsigned int i = 0; i < scene->mNumMaterials; i++) {
    ...
    if (aiMat->GetTexture(aiTextureType_DIFFUSE, 0, &path, 0, 0, 0, 0, 0) ==
      ...
      unsigned char color[4] = { static_cast<unsigned char>(0xff * diff.r), static_cast<unsigned char>(0xff * diff.g), static_cast<unsigned char>(0xff * diff.b), 0xff };
    ...
    if (aiMat->GetTexture(aiTextureType_SPECULAR, 0, &path, 0, 0, 0, 0, 0) ==
      ...
      unsigned char color[4] = { static_cast<unsigned char>(0xff * spec.r), static_cast<unsigned char>(0xff * spec.g), static_cast<unsigned char>(0xff * spec.b), 0xff };
