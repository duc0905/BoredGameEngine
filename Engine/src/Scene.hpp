...
/**
 * A logic processing component of a scene. Have access to the ActorManager
 * (ECS) of its scene Only use Components to process logic
 */
class Module {
  friend class Scene;
 ...
 private:
  void Attach(Scene* scene) { m_scene = scene; }
...
class Scene {
 ...
 public:
  ...
  std::shared_ptr<T> AddModule(Args&&... args) {
    ...
    m->Attach(this);
    std::cout << "Mod ptr: " << (unsigned int)m.get() << std::endl;
    m_mods.push_back(m);
    return m;
  }
   ...
   */
  template <class T>
  std::shared_ptr<T> AddDLModule(
      const std::string& modulePath,
      const std::string& allocSymbol = CREATE_INSTANCE,
      const std::string& deleteSymbl = DELETE_INSTANCE) {
    std::shared_ptr<Util::DLLoader> loader =
        std::make_shared<Util::DLLoader>(modulePath);
    loader->Load();
  ...
  Bored::ActorManager& GetActorManager() { return m_actorManager; }
